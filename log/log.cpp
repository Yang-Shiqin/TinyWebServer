...
// 日志创建，同步异步写入方式判断(通过max_queue_size是否大于等于1)
...
bool Log::init(const char *file_name, int close_log, int log_buf_size, int split_lines, int max_queue_size)
{
    ...
    {
        strcpy(log_name, p + 1);    // 拷贝文件名
        strncpy(dir_name, file_name, p - file_name + 1);    // 拷贝目录名
...
void Log::write_log(int level, const char *format, ...)
{
    ...
    if (m_is_async && !m_log_queue->full()) // [ ] TODO: 满了咋办
    {
